...
# %%
def encoder(input_shape=(IMAGE_SIZE[0], IMAGE_SIZE[1], 3)):
    ...
    x = tf.keras.layers.Conv2D(
        width, kernel_size=3, activation="relu", padding="same")(block_1_output)
    x = tf.keras.layers.Conv2D(
        width, kernel_size=3, activation="relu", padding="same")(x)
    block_2_output = tf.keras.layers.add([x, block_1_output])

    x = tf.keras.layers.Conv2D(
        width, kernel_size=3, activation="relu", padding="same")(block_2_output)
    x = tf.keras.layers.Conv2D(width, kernel_size=3,
                               activation="relu", padding="same")(x)
    block_3_output = tf.keras.layers.add([x, block_2_output])

    x = tf.keras.layers.Conv2D(
        width, kernel_size=3, activation="relu")(block_3_output)
