...
print(STEPS_PER_EPOCH, UNLABELED_BATCH_SIZE, LABLED_BATCH_SIZE_TRAIN,LABLED_BATCH_SIZE_TEST)
...
# %% [markdown]
# Reading labeled data from disk
...
# %%
def get_encoder(input_shape = (IMAGE_SIZE[0],IMAGE_SIZE[1],3)):
    inputs = tf.keras.Input(shape = input_shape, name="img")
    x = tf.keras.layers.Conv2D(width, kernel_size = 3, activation="relu")(inputs)
    x = tf.keras.layers.Conv2D(width,kernel_size =  3, activation="relu")(x)
    block_1_output = tf.keras.layers.MaxPooling2D(3)(x)

    x = tf.keras.layers.Conv2D(width, kernel_size = 3, activation="relu", padding="same")(block_1_output)
    x = tf.keras.layers.Conv2D(width,kernel_size =  3, activation="relu", padding="same")(x)
    block_2_output = tf.keras.layers.add([x, block_1_output])

    x = tf.keras.layers.Conv2D(width,kernel_size =  3, activation="relu", padding="same")(block_2_output)
    x = tf.keras.layers.Conv2D(width,kernel_size =  3,  activation="relu", padding="same")(x)
    block_3_output = tf.keras.layers.add([x, block_2_output])

    x = tf.keras.layers.Conv2D(width, kernel_size = 3, activation="relu")(block_3_output)
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    x = tf.keras.layers.Dense(width, activation="relu")(x)
    x = tf.keras.layers.Dropout(0.5)(x)
    x = tf.keras.layers.Flatten()(x)
    outputs = tf.keras.layers.Dense(width, activation="relu")(x)
  #outputs = tf.keras.layers.reshape(target_shape = (-1, *input_shape))(outputs)

    model = tf.keras.Model(inputs, outputs, name="return")
    return model
...
pretraining_model.compile(
    contrastive_optimizer=tf.keras.optimizers.Adam(learning_rate = 0.0001),
    probe_optimizer=tf.keras.optimizers.Adam(learning_rate = 0.0001),
)
...
finetuning_history = finetuning_model.fit(
    labeled_train_data, epochs=num_epochs, validation_data=labeled_val_data
)
print(
    "Maximal validation accuracy: {:.2f}%".format(
...
ax.xaxis.set_ticklabels(defect_type)
ax.yaxis.set_ticklabels(defect_type)
...
lw = 2
lb = LabelBinarizer()
yenc = lb.fit(test_image_labels).transform(test_image_labels)
yenc

# %%
for i in range(n_classes):
    fpr[i], tpr[i], _ = roc_curve(yenc[:, i], ypred[:, i])
    roc_auc[i] = auc(fpr[i], tpr[i])


fpr["micro"], tpr["micro"], _ = roc_curve(yenc.ravel(), ypred.ravel())
...
for i, color,dt in zip(range(n_classes), colors,defect_type):
    plt.plot(
        fpr[i],
        tpr[i],
        color=color,
